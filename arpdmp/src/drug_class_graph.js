...
export class DrugClassGraph extends Component {
	constructor(props){
		super();
	...
	render(props){
		...
		if(props.zip == undefined){
			return(<span>Click a zip.</span>);
		}
		if(props.zip != this.state.zip && this.state.fetching == false){
			this.setState({ fetching: true });
			window.fetch("http://localhost:8080/api/zip_class?zip=" + props.zip)
				    .then(response => response.json())
						.then(data =>
							this.setState({
								...
								zip: props.zip,
								height: (data.length * barHeight)+15,
								fetching: false
							})
						);
						...
						{this.state.data.map( (datum, index) => (
							...
							<rect fill="palevioletred"
								    ...
								    x={0}
										...
										height={barHeight}/>
							<text x={15}
							      y={yScale(index)+(barHeight/2)+1}>{datum.drug_class}</text>
							</g>
						))}
					</g>
				</svg>
