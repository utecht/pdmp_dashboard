...
export class PrescriptionsGraph extends Component {
	constructor(props){
		super();
	...
	render(props){
		if(props.zip == undefined){
			return(<span>Hover over a zip.</span>);
		}
		if(props.zip != this.state.zip && this.state.fetching == false){
			this.setState({ fetching: true });
			window.fetch("http://localhost:8080/api/zip?zip=" + props.zip)
						...
						.then(data =>
							this.setState({
								data: data.map(d => {
										...
										newD['prescriptions'] = d.prescriptions;
									  return newD;
									 })
									.sort(function(x, y){
	  ...
	  var yScale = d3.scaleLinear()
	    .domain([0, d3.max(this.state.data, function(d) { return +d.prescriptions; })])
					...
					<g transform="translate(60,10)">
						...
						{this.state.data.map( (datum) => (
							<rect fill="steelblue"
								    ...
								    x={xScale(datum.date)}
										width={barWidth}
										y={yScale(datum.prescriptions)}
										height={this.state.height - yScale(datum.prescriptions)}/>
						))}
						<text x="20" y="20" style="font-size: 15px;" alignment-baseline="middle">Zip Code {props.zip}XX</text>
						<rect x="20" y="25" width="15" height="15" style="fill: steelblue;"/>
						<text x="40" y="40" style="font-size: 15px;" alignment-baseline="middle">Prescriptions</text>
					</g>
				</svg>
			</div>
		)
